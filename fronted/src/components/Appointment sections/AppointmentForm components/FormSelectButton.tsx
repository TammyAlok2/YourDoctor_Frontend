...
interface FormSelectedButtonProps {
  first: string;
  second: string;
}


const FormSelectedButton = ({ first, second }: FormSelectedButtonProps) => {
  ...
  return (
    <div>
      ...
      >
        <h1>{selectval}</h1>
        <Image width={22} height={20} src={toggleicon} alt="" />
      </div>
      {toggle && (
        <div
          className="cursor-pointer absolute border-[0.1rem] border-black rounded-lg mt-[0.3rem] transition-[3s]"
        >
          <div
