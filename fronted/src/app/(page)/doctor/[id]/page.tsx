  ...
  <div className="animate-pulse">
    ...
    <section className="mb-12">
      <div className="flex flex-col sm:flex-row justify-between space-y-4 sm:space-y-0 sm:space-x-4">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="bg-gray-200 p-4 rounded-lg shadow-lg flex-1 h-36"></div>
        ))}
      </div>
    </section>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start">
            <div className="text-black space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-[0.9rem]">
                <a className="list-none text-gray-600">Emergency Fee1: <span className="text-teal-700">{doctor.fees?.emergencyFee1}rs</span></a>
                <a className="list-none text-gray-600">Emergency Fee2: <span className="text-teal-700">{doctor.fees?.emergencyFee2}rs</span></a>
                <a className="list-none text-gray-600">First Visit Fees: <span className="text-teal-700">{doctor.fees?.firstVisitFee}rs</span></a>
                <a className="list-none text-gray-600">Second Visit Fee: <span className="text-teal-700">{doctor.fees?.secondVisitFee}rs</span></a>
                <a className="list-none text-gray-600">Visit Under 7 Days Fees: <span className="text-teal-700">{doctor.fees?.visitUnder7DaysFee}rs</span></a>
              </div>
            </div>
            <div className="flex flex-col items-center mt-4 md:mt-0">
              <div className="relative w-24 h-24 rounded-full mb-2">
                <div className={`${doctor.status === false ? "" : "border-4 rounded-full border-[#0A8E8A] w-[7rem] h-[7rem] flex items-center justify-center p-[0.2rem] mx-auto"}`}>
                  ...
                  <div 
                    ...
                    style={{backgroundColor: doctor.status === false ? 'transparent' : '#54FC05'}}
                  ></div>
                </div>
              </div>
