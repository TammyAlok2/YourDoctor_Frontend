...
// Define the type of a Doctor object
type Doctor = {
  _id: string;
  fullName: string;
  specialist: string;
...
type FirstDoctorsSectionProps = {
  setData: Dispatch<SetStateAction<Doctor[]>>;
  filteredData: Doctor[];
};

const FirstDoctorsSection = ({ setData, filteredData }: FirstDoctorsSectionProps) => {
  ...
  const getAllDoctor = async () => {
    try {
      ...
      if (storedDoctors) {
        ...
        setData(parsedDoctors.slice(0, 3));
      } else {
            ...
            <div className="flex flex-col gap-[1rem] w-[16rem]">
              ...
              <ul className="text-gray-600 list-none">
                <a className="list-none text-gray-600">
                  Fees: <span className="text-teal-700">{userData?.fees && userData?.fees?.firstVisitFee + "rs"}</span>
                </a>
              </ul>
            </div>
            <div className="ml-auto flex flex-col items-end sm:items-start relative gap-[0.8rem] w-[45%] xs:w-[100%] sm:w-auto">
              <div className="w-[6rem] h-[6rem] rounded-full overflow-hidden items-end ml-auto relative">
                <div className={`${userData?.status === false ? "" : "border-4 rounded-full w-22 h-22 border-[#0A8E8A] flex text-center justify-center p-[0.2rem] mx-auto"}`}>
                  {userData?.avatar && <Image src={userData?.avatar?.secure_url} alt={"Doctor Avatar"} width={100} height={100} className="rounded-full object-cover" />}
                </div>
                <div className={`absolute right-2 w-[0.8rem] animate-ping rounded-full bottom-3 h-[0.8rem]`} style={{ backgroundColor: `${userData?.status === false ? "" : "#54FC05"}` }}></div>
              </div>
